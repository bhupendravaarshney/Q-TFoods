...
function supplement(code: Extract<FinanceP2Code, `FIN-${string}`>, title: string, description: string, listPath: string, kind: string, numberKey: string, columns: GovernedP2Config['collections'][number]['columns']): GovernedP2Config {
  ...
  return {
    ...
    creators: [{ action: 'CREATE', label: `New ${title.toLowerCase().replace('finance ', '')}`, path: `/api/v1/finance/${pathSegment}`, available: (workspace) => code === 'FIN-SUP' || rows(workspace, 'accounts').length >= 2, help: 'The command is prefilled from live scoped lookup data; edit business facts before submission.', template: (workspace) => supplementTemplate(code, workspace) }],
...
function supplementTemplate(code: FinanceP2Code, workspace: P2Workspace): unknown {
  if (code === 'FIN-SIM') return { simulation_number: commandNumber('SIM'), name: '', description: null, as_of_date: today(), lines: balancedLines(workspace) };
