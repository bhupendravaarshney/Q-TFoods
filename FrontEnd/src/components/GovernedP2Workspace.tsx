...
function RelatedRows({ values }: { values: unknown[] }) {
  ...
  const rows = values.filter((value): value is Record<string, unknown> => Boolean(value) && typeof value === 'object'); const keys = Array.from(new Set(rows.flatMap((row) => Object.keys(row)))).filter((key) => !key.endsWith('_id') && !['id', 'company_id', 'plant_id', 'created_at', 'updated_at'].includes(key)).slice(0, 6);
