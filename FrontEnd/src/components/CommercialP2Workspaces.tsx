...
const edit = (label: string, help: string, path: string, body: unknown, record: P2Record): P2ActionSpec => ({ editor: { label, help, path, body, expectedVersion: record.record_version } });
...
export const commercialP2Configs: Record<CommercialP2Code, GovernedP2Config> = {
  'CRM-LEAD': {
    ...
    creators: [{ action: 'CREATE', label: 'New lead', path: '/api/v1/sales/leads', help: 'Complete the enquiry and contact facts. Server policy owns status and event history.', template: (workspace) => {
    ...
    resolveAction: (action, record, workspace) => {
      ...
      if (action === 'UPDATE') return edit('Update lead', 'Only editable enquiry facts are sent; identity and history remain immutable.', base, leadBody(record), record);
      if (action === 'QUALIFY') return run(`${base}/qualify`, 'Lead qualified.', record);
      if (action === 'CONVERT') return edit('Convert lead', 'Select the governed customer party that owns the converted opportunity.', `${base}/convert`, { customer_party_id: value(record, 'customer_party_id', id(first(workspace, 'customers'))) }, record);
      if (action === 'CLOSE_WON' || action === 'CLOSE_LOST') return edit(action === 'CLOSE_WON' ? 'Close lead as won' : 'Close lead as lost', 'Record the outcome and a reviewable commercial reason.', `${base}/close`, { outcome: action === 'CLOSE_WON' ? 'WON' : 'LOST', reason: '' }, record);
      return null;
    },
  },
  'CRM-PRICE': {
    ...
    resolveAction: (action, record) => {
      ...
      if (action === 'UPDATE') return edit(contract ? 'Update contract' : 'Update price list', 'Submit the complete replacement line set under optimistic locking.', base, contract ? contractBody(record) : priceBody(record), record);
      if (action === 'ACTIVATE') return run(`${base}/activate`, contract ? 'Contract activated.' : 'Price list activated.', record);
      if (action === 'RETIRE' || action === 'CLOSE') return edit(contract ? 'Close contract' : 'Retire price list', 'A reason is required and retained in the audit event.', `${base}/${contract ? 'close' : 'retire'}`, { reason: '' }, record);
      return null;
    },
  },
  'CRM-ORDER': {
    ...
    creators: [{ action: 'CREATE', label: 'New sales order', path: '/api/v1/sales/orders', available: (workspace) => Boolean(first(workspace, 'customers') && first(workspace, 'items')), help: 'Choose governed IDs from the provided lookups. The server resolves the applicable price and credit snapshot.', template: (workspace) => { const contract = first(workspace, 'contracts'); const customer = contract ? { id: value(contract, 'customer_party_id') } : first(workspace, 'customers'); const item = first(workspace, 'items'); return { order_number: number('SO'), customer_party_id: id(customer), sales_lead_id: null, sales_contract_id: id(contract) || null, sales_price_list_id: null, order_date: today(), requested_delivery_date: future(7), notes: null, lines: [{ item_id: id(item), uom_code: value(item, 'base_uom', 'EA'), quantity: '1', discount_percent: '0' }] }; } }],
    resolveAction: (action, record) => { const base = `/api/v1/sales/orders/${record.id}`;
      if (action === 'UPDATE') return edit('Update sales order', 'Replace editable header and line facts while the order remains draft.', base, orderBody(record), record);
      if (action === 'CONFIRM') return run(`${base}/confirm`, 'Sales order confirmed after pricing and credit checks.', record);
      if (action === 'AMEND') return edit('Amend confirmed order', 'A new immutable revision is created from this complete replacement payload.', `${base}/amend`, { reason: '', ...orderBody(record) }, record);
      if (action === 'CANCEL') return edit('Cancel sales order', 'Cancellation releases active reservations and records the reason.', `${base}/cancel`, { reason: '' }, record);
      if (action === 'ALLOCATE') return edit('Allocate sales order', 'FEFO stock selection and reservation quantities remain server authoritative.', `/api/v1/dispatch/orders/${record.id}/allocations`, { allocation_number: number('ALLOC') }, record);
      return null;
    },
  },
  'CON-WORK': {
    ...
    creators: [{ action: 'CREATE', label: 'New contracted work', path: '/api/v1/sales/third-party-work', available: (workspace) => Boolean(first(workspace, 'providers')), help: 'Create a scoped provider engagement; release and completion are separate commands.', template: (workspace) => ({ work_number: number('TPW'), sales_order_id: id(first(workspace, 'sales_orders')) || null, provider_party_id: id(first(workspace, 'providers')), work_type: 'CO_PACKING', description: '', expected_start_date: today(), expected_end_date: future(7), agreed_cost: '0' }) }],
    resolveAction: (action, record) => { const base = `/api/v1/sales/third-party-work/${record.id}`; if (action === 'RELEASE') return run(`${base}/release`, 'Third-party work released.', record); if (action === 'COMPLETE') return edit('Complete contracted work', 'Record the final actual cost before closing the engagement.', `${base}/complete`, { actual_cost: value(record, 'agreed_cost', '0') }, record); if (action === 'CANCEL') return edit('Cancel contracted work', 'Record a reviewable cancellation reason.', `${base}/cancel`, { reason: '' }, record); return null; },
  },
  'DSP-PICK': {
  ...
  'DSP-LOAD': {
    ...
    creators: [{ action: 'CREATE', label: 'New shipment', path: '/api/v1/dispatch/shipments', available: (workspace) => Boolean(first(workspace, 'allocations')), help: 'Only a picked allocation can become a shipment.', template: (workspace) => shipmentCommand(id(first(workspace, 'allocations'))) }],
  ...
  'RET-CASE': {
    ...
    creators: [{ action: 'CREATE', label: 'New customer claim', path: '/api/v1/sales/customer-claims', available: (workspace) => Boolean(value(first(workspace, 'shipments'), 'first_line_id')), help: 'The first eligible shipment line is prefilled. Add or replace lines using IDs returned in the shipment lookup.', template: (workspace) => { const shipment = first(workspace, 'shipments'); return claimCommand(shipment, String(value(shipment, 'first_line_id', ''))); } }],
    resolveAction: (action, record) => { const base = `/api/v1/sales/customer-claims/${record.id}`; if (action === 'RECEIVE') return edit('Receive returned goods', 'Record return receipt notes; stock quarantine remains server controlled.', `${base}/receive`, { notes: null }, record); if (action === 'RESOLVE') return edit('Resolve customer claim', 'Choose CREDIT, REPLACEMENT or REJECT and record the decision evidence.', `${base}/resolve`, { resolution_type: value(record, 'requested_resolution', 'CREDIT'), credit_amount: value(record, 'requested_resolution') === 'CREDIT' ? value(record, 'requested_amount', '0') : null, notes: '' }, record); return null; },
  },
  'FIN-AR': {
...
function shipmentAction(action: string, record: P2Record): P2ActionSpec | null {
  ...
  if (action === 'DISPATCH') return edit('Dispatch shipment', 'Dispatch posts stock and creates the controlled sales invoice.', `${base}/dispatch`, { invoice_number: number('INV') }, record);
  if (action === 'CANCEL') return edit('Cancel shipment', 'Record the cancellation reason before loading is finalized.', `${base}/cancel`, { reason: '' }, record);
  if (action === 'POD') return edit('Complete proof of delivery', 'Record delivered or failed outcome and its evidence.', `${base}/pod`, podCommand(record), record);
  if (action === 'CLAIM') return edit('Create customer claim', 'Tie the claim to exact shipment-line quantities.', '/api/v1/sales/customer-claims', claimCommand(record, String(value(detailRows(record)[0], 'id', ''))), record);
  return null;
}
