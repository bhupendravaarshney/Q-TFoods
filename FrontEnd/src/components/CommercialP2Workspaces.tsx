...
export const commercialP2Configs: Record<CommercialP2Code, GovernedP2Config> = {
  ...
  'CRM-PRICE': {
    ...
    creators: [
      { action: 'CREATE_PRICE_LIST', label: 'New price list', path: '/api/v1/sales/price-lists', available: (workspace) => Boolean(first(workspace, 'items')), help: 'Start with an active catalog item, then add or update price lines in the form.', template: (workspace) => { const item = first(workspace, 'items'); return { list_number: number('PL'), name: '', effective_from: today(), effective_to: future(30), notes: null, lines: [{ item_id: id(item), uom_code: value(item, 'base_uom', 'EA'), minimum_quantity: '1', unit_price: '0', maximum_discount_percent: '0', tax_rate: '0' }] }; } },
  ...
  'RET-CASE': {
    ...
    creators: [{ action: 'CREATE', label: 'New customer claim', path: '/api/v1/sales/customer-claims', available: (workspace) => Boolean(value(first(workspace, 'shipments'), 'first_line_id')), help: 'The first eligible shipment line is selected automatically. Add or update return lines in the form.', template: (workspace) => { const shipment = first(workspace, 'shipments'); return claimCommand(shipment, String(value(shipment, 'first_line_id', ''))); } }],
